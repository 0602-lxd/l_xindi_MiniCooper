        ...
        methods: {
            getData () { // 获取所有数据
                this.list.forEach((element, index) => {
                    fetchData(`./includes/index.php?id=${index+1}`).then(data => {console.log(data);  if(data.code){alert(data.msg);}else{this.initHtml(data[0]);}  }).catch(err => {console.log(err);  alert(err); }); //request data index.php
                });
            },
            initHtml (data) {
